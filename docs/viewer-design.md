...
**Swift + WKWebView の薄い殻**を推す。

- `NSWindow` の level を `.floating` にすれば本物の最前面固定になる
- 中身は既存のローカルサーバ（127.0.0.1:8090）をそのまま表示するだけ
- **パネルはマイクを使わない**ので、WKWebView でマイク許可に苦労する問題が起きない
- この repo は既に `speech_helper.swift` を素の `swiftc` でビルドしている。
  新しい言語やツールチェーンを増やさずに済む

Electron（100MB 超）や Tauri（Rust が必要）を持ち込む必要はない。

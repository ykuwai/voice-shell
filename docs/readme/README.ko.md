...
- **무료로 써 볼 수 있고, 따로 설치할 것도 없습니다.** 기본 인식은 브라우저가
  처리하므로 모델을 내려받거나 기다릴 것도, API 키도 필요 없습니다. 완전히
  내 컴퓨터 안에서만 처리하고 싶어지면 설정 하나만 바꿔서 Apple이나
  Whisper의 온디바이스 인식으로 전환할 수 있습니다. 새로 익힐 것은 없습니다.
...
기본값은 브라우저의 Web Speech API로, 가장 빠르게 시작할 수 있는
방식이자 목소리를 이 컴퓨터 밖으로 내보내는 유일한 방식입니다.
나머지 두 방식은 전부 로컬에서 끝나고, 전환도 설정 하나면 됩니다.
어느 방식이 듣고 있는지는 항상 창에 나타납니다.
...
|---|---|
| "음소거" | 마이크가 꺼집니다 |
| "음소거 해제" | 마이크가 돌아옵니다 (온디바이스 방식은 이때도 계속 듣고 있습니다. 브라우저는 아닙니다) |

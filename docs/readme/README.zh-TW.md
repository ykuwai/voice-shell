...
不需要按傳送按鈕。說完話過 3 秒，內容就會自動傳給 Claude Code。\
靜音、取消、切換傳送對象，也都能用聲音完成。

### 2. 常用的詞可以登錄進字典

像「Cloud Code → Claude Code」這樣容易聽錯的詞，可以自動改正。\
人名、公司名稱、服務名稱等等，也都能輕鬆登錄。

### 3. 完全免費，用起來也安心

高效能的語音輸入完全免費，沒有任何收費項目。\
筆電可以用瀏覽器的語音辨識，Mac 或高效能的電腦則可以全部在本機處理完成。
...
Mac（macOS 26 以後）可以使用 Apple 的語音辨識，耗電少，速度也快。\
...
在多個 Claude Code 裡執行 `/voice-shell`，畫面上方就會加上編號排列出來。\
...
|---|---|
| 「靜音」 | 關閉麥克風 |
| 「解除靜音」 | 開啟麥克風（只限本機語音辨識）。<br>使用瀏覽器的語音辨識時，請按畫面上的麥克風開啟 |
...
在設定的「多台電腦一起使用」裡，替每台取個「公司」「家裡」之類的名字，說「公司靜音」時就只有那一台會靜音。
...
判斷說完話的音量，可以用麥克風下方的滑桿調整。周圍比較吵的時候，請調高一點。
...
- [SKILL.md](../../skills/voice-shell/SKILL.md) 　使用方式和行為的步驟

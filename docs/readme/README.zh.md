...
- **不用按任何东西来发送。** 大多数语音工具是把内容填进文本框，然后等您去点
  ...
  窗口。
- **免费试用，不用额外装东西。** 默认用浏览器识别，不用下模型，不用等，也
  不用 API 密钥。想让一切都留在本地的时候，只要改一个设置，就能切换到
  Apple 或 Whisper 的设备端识别，不用重新学任何东西。
...
默认用的是浏览器自带的 Web Speech API，是最快能上手的方式，也是唯一
会把您的声音送到这台机器以外的方式。另外两种全在本地跑完，切换只需要
改一个设置。当前用的是哪一种，窗口里始终能看到。
...
|---|---|
| 「静音」 | 麦克风关闭 |
| 「解除静音」 | 麦克风恢复（本地识别的方式在这时仍在听，浏览器不会） |

...
話すのをやめたいときは「ミュート」と言えば、マイクが切れます。
...
|---|---|
| 「ミュート」 | マイクを切ります |
| 「ミュート解除」 | マイクを戻します（ローカルの音声認識のときだけ。ブラウザの音声認識では、画面のマイクを押して戻します） |
...
> **音声コマンドの一覧**
>
> すべての言い方は、画面の電球アイコンから確認できます。自分の言い方を足したり、使わないものを切ったりもできます。

### 複数台モード

2台のパソコンで同時に使っていると、「ミュート」と言っただけで両方が切れてしまいます。
設定の「複数台で使う」で、それぞれに「仕事用」「趣味」のような呼び名を付けておくと、「仕事用ミュート」と言ったほうだけが切れます。

## ✨ 便利な機能や設定

### 1. 送信のタイミングを調整できる

最初は、話し終わってから3秒で送られます。考えながら話したいなら、5秒や10秒にもできます。
...
複数のセッションで使っていて、別のセッションに送ってしまったときは、送信先をマウスで選び直せます。
...
| `Shift` + `L` | 即時にします |
| `Shift` + `H` | 手直しにします |
| `Shift` + `E` | 今話した内容だけ手直しに回します |
| `Shift` + `1`〜`9` | 送信先を番号で切り替えます |
| `Shift` + `Backspace` | 未送信の内容を捨てます |
| `Ctrl` + `Enter`（Mac は `Cmd` + `Enter`） | 手直し中の内容を送ります |
| `,` | 設定を開きます |
| `.` | 辞書を開きます |

...
- **無料で、追加のインストールなしで試せます。** 既定の認識はブラウザが
  行うので、モデルのダウンロードも待ち時間もAPIキーも要りません。
  ローカルだけで完結させたくなったら、設定をひとつ変えるだけでApple・
  Whisperのオンデバイス認識に切り替えられます。使い方は変わりません。
- **ただの音声入力ツールではありません。** ミュート、手直しと即時の
  切り替え、直前の取り消し、どのセッションに届けるかの選択、これも全部
  声でできます。詳しくは下の「音声コマンド」をご覧ください。画面には、
  聞き取った内容がそのまま伸びていきます。
...
既定のWeb Speech API（ブラウザ標準の音声認識）が、いちばん手早く
始められて、音声を外へ出す唯一の方式です。残りの2つはこの端末の中で
完結し、切り替えは設定ひとつです。どのやり方が動いているかは、いつも
画面に出ています。

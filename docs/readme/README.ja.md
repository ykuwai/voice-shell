...
どこで認識するかは Chrome が決めます。その言語のモデルがパソコンにあれば手元で認識することがあり、なければ音声は Google のサーバーへ送られます。
...
|---|---|
| 「ミュート」 | マイクをオフにします |
| 「ミュート解除」 | マイクをオンにします。ローカル音声認識と、この端末内で認識するブラウザの音声認識が聞き取ります。<br>通常のブラウザの音声認識では、画面のマイクを押してオンにします |
| 「2番」「セッション2」 | 複数のセッションを使っているときに送信先の番号を伝えると、送信先を切り替えられます |
| 話し終わりに「キャンセル」 | 今話した内容を取り消します |
| 話し終わりに「手直し」 | 一時的に手直しモードになります |
| 「手直し」「即時」 | 送信モードを切り替えます |
...
電球アイコンを開き、コマンド一覧の下にある「複数台で使う」をオンにして、それぞれに「仕事用」「趣味」のような呼び名を付けておくと、「仕事用ミュート」と言ったほうだけがミュートされます。

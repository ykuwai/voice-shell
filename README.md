...
設定の「認識するのは」から選ぶ。**既定は「このブラウザ」** — 何も入れずに
動くので、まず誰でも試せる。手元で完結させたくなったら、入っているモデルへ
切り替える（入っていないものは選択肢に出ない）。

| やり方 | 何が要るか | 音声の行き先 |
|---|---|---|
| **このブラウザ（既定）** | Chrome だけ | **Google のサーバ** |
| Apple のオンデバイス | macOS 26 以降 | 手元だけ |
| Whisper | CPU でも動く（固有名詞に強い） | 手元だけ |
| Qwen3-ASR | GPU 約12GB / Apple Silicon | 手元だけ |

両方を同時には動かさない（同じ発話が2回届くため）。切り替えると、
モデル側は自動で積み下ろしされる。ブラウザで認識している間は、
積むものが無いので電源ボタンは出ない。

ブラウザの認識はデーモンが動いていなくても使える。7〜10 秒黙るとセッションが切れる仕様だが、**黙っているあいだに
...
別々のことを並行して進めているとき（アプリの開発と、この道具の開発、など）、
それぞれで音声モードを立ち上げておき、**どれへ届けるかをビューアから選べる**。

聞いているセッションが2つ以上になると、画面の上に送信先が並ぶ。

```
[ すべて ] [ voice-shell のビューア刷新 ] [ 認証まわりを直す ]
```

**名前は勝手に付く。** 最初はフォルダ名で、エージェントがその会話に題名を
付けた時点でそちらへ切り替わる（会話が進めば題名も追う）。同じフォルダから
2つ立てたときは `(2)` が付く。
...
| そのほか | フォルダ名。`VOICE_SHELL_NAME` で名乗るか、下のコマンドで付ける |

```bash
voice-shell.sh name "認証まわりを直す"    # このセッションの表示名を手で付ける
voice-shell.sh name ""                    # 自動の題名に戻す
```

送信先を選ぶと、発話ログの行に宛先が付き、`listen` が自分宛てと全員宛てだけを
通す。ログ自体は1本のままなので、ビューアの履歴はこれまでどおり全部見える。

選んでいた相手が終了したら、**画面で知らせて1つ前の生きているセッションへ移す**。
黙って宙に浮かせると、話しても届かない状態に気づけないため。

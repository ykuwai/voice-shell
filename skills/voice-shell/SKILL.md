...
- **声だけで操作できる** — 「ミュート」「ミュート解除」（`mute` / `unmute`）、
  「2番目」「2番」「2に切り替え」（`switch to 2`）で送信先の変更、「全員に送る」
  （`send to all`）。**その一言だけ**喋ったときに限る（文の中で言っても効かない）。
  数の読みの揺れ（に／ツー／two）は吸収する。切り替わると短い音が鳴る。送信先の
  番号は画面のチップに出ている順で、切っている間も切り替えられる。手元のモデルを
  選んでいるときだけ（ブラウザ認識は切ると音声を手放すので、切った後の合図を聞けない）
...
| ひと続きの上限 | これより長く話し続けると次の息継ぎで区切る。0 なら区切らない | 30 秒 |

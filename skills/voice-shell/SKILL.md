...
4. ブラウザ自動操作の道具（claude-in-chrome 等）が使えるなら、ビューアを
   自動で開き、手前に浮かせる。使えない環境では飛ばしてよい（URL を
   案内するだけでよい）:

   - `http://127.0.0.1:8090` を新しいタブで開く
   - 画面右上の「手前に浮かせる」ボタン（`documentPictureInPicture` を使う。
     ヘッダのアイコン）をクリックする。`'documentPictureInPicture' in window`
     が false のブラウザ（Chrome 以外）ではボタン自体が隠れているので、
     その場合は諦めて通常のタブのまま使う
   - 浮かせたウィンドウは常に最前面に来るので、キーボードを使わず他の作業を
     進めながら認識状況を確認できる。これが目的なので、開いた通常タブは
     浮かせたあとに閉じてよい
...
デーモン側（Qwen3-ASR / Apple / Whisper、音声は手元から出ない）のほかに、
**Chrome の認識**を使う道がある（設定の「このブラウザで認識する」）。モデルを
積まないので非力な端末でも動き、デーモンが動いていなくても使える。

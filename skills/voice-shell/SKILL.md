...
- **声だけで操作できる** — 「ミュート」「ミュート解除」（`mute` / `unmute`）、
  「2番」「2に切り替え」（`switch to 2`）で送信先の変更、「全員に送る」（`send to all`）。
  **その一言だけ**喋ったときに限る（文の中で言っても効かない）。切り替わると短い音が
  鳴る。送信先の番号は画面のチップに出ている順。手元のモデルを選んでいるときだけ
  （ブラウザ認識は切ると音声を手放すので、切った後の合図を聞けない）

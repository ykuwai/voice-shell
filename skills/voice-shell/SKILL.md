...
1. 起動する:
   ...
   **ブラウザ自動操作の道具（claude-in-chrome 等）が使えない環境**では、
   ...
   何が使えるか知りたいときは `voice-shell.sh engines`。ユーザーが
   「手元だけで認識したい」「クラウドに送りたくない」と言ったときも
   `--engine auto`（または `--engine apple` のように明示）を使う。
...
3. **ビューアを開いて手前に浮かせる。** ブラウザ認識のときは
   **画面を開くまで一切届かない**ので、これは必須。

   - `http://127.0.0.1:8090` を新しいタブで開く
   - ヘッダの「手前に浮かせる」アイコンをクリックする（`documentPictureInPicture`
     を使う。Chrome 以外ではボタン自体が隠れているので、その場合は通常の
     タブのままでよい）
   - 浮かせたら、元のタブは閉じてよい

   ブラウザ自動操作の道具が無ければ、URL を案内して開いてもらう。
...
4. 発話ログを Monitor で監視する。**`persistent: true` を必ず指定する**
   ...
   セッション終了）すれば登録も消える。生の `tail -F` ではこの登録が
   行われず、多重起動が静かに再発する。
...
**`voice-shell.sh listen` を pkill で止めるときは注意する** — 自分の Monitor も
同じパターンに一致するので、巻き添えで落ちる（実際に3回やった）。
...
聞き手が2つ以上になると、ビューアの上に**送信先**が並び、ユーザーが選べる。
選ばれていない発話は届かない（これは正常。取りこぼしではない）。
